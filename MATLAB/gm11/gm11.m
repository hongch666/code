function [ result, x0_hat, relative_residuals, eta ] = gm11( x0, predict_num )
    ...
    %   result：预测值
    %   x0_hat：对原始数据的拟合值
    %   relative_residuals：对模型进行评价时计算得到的相等残差
    %   eta：对模型进行评价时计算得到的级比偏差
    n = length(x0); %数据长度
    x1 = cumsum(x0); %计算一次累加值
    z1 = (x1(1:end-1) + x1(2:end)) / 2; %计算紧邻均值生成数列
    %将第二项开始的x0当成y，z1当成x，来进行一元回归
    y = x0(2:end);
    x = z1;
    ...
    disp('****************分割线****************')
    x0_hat = zeros(n,1); %x0_hat存储x0序列的拟合值
    x0_hat(1) = x0(1);
    for m = 1:n-1
    ...
    relative_residuals = abs(absolute_residuals)./x0(1:end-1);
    %计算级比和级比偏差
